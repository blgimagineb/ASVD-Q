...
class SVDLinear(nn.Module):
    ...
    def from_linear(
        ...
        if act_aware:
            ...
            in_scale_matrix += 1e-6
            in_scale_matrix = in_scale_matrix ** alpha
            w = w * in_scale_matrix.view(1, -1)
            w = w * out_scale_matrix.view(-1, 1)
        Us = []
        Ss = []
        Vs = []
        try:
        ...
        if act_aware:
            V = V / in_scale_matrix.view(-1, 1)
            U = U / out_scale_matrix.view(-1, 1)
        Us = [U]
        Ss = [S]
        Vs = [V]
